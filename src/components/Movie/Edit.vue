<template>
  <div class="grow flex flex-col items-center pt-12 bg-gradient-to-br from-purple-200 to-indigo-400">
  <div class="mt-6 rounded-3xl transition duration-300 w-full md:p-6 h-full">
    ...
    <div class="w-full flex flex-row justify-center pt-8 space-x-4">
      <button class="w-32 h-10 rounded-lg border-[1px] border-black hover:bg-violet-600 disabled:border-black/10 disabled:text-black/10" :disabled="!can_submit" @click="submit">
        {{translate("Submit", "提交")}}
      </button>
      <button class="w-32 h-10 rounded-lg border-[1px] border-black hover:bg-violet-600" @click="click_back">
        {{translate("Return", "返回")}}
      </button>
    </div>
  </div>
...
import {video_detail, video_update} from "@/utility/backend";
...
onMounted(async()=>{
  let detail = await video_detail(route.query.id)
  if(detail.status_code !== 0){
    alert_operator.push_alert("error", translate("failed to get detail content", "获取详情内容失败"));
    return;
  }
  ...
  video.poster_path = data.poster_path;
  video.video_type = data.video_type;
  video.region = data.region;
  video.link = data.link;
  video.link_type = data.link_type;
  video.backdrop_path = data.backdrop_path;
});
...
async function submit(){
  ...
  let result = await video_update(data);
  if(result.status_code !== 0){
    alert_operator.push_alert("error", translate(`Failed to update video, reason:${result.status_message}`, `更新视频失败，理由:${result.status_message}`));
  }else{
    alert_operator.push_alert("ok", translate(`Success to update video`, `更新视频成功`));
    setTimeout(()=>{
      router.push("/movie")
    },2000);
  }
