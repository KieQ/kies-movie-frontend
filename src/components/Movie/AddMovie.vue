<template>
  <div class="mt-6 rounded-3xl transition duration-300 md:p-6 h-full">
    ...
    <div class="flex flex-col px-1 md:px-8 py-3 h-full min-w-fit items-center mt-3 bg-purple-50 rounded-lg space-x-1 md:space-y-4">
      ...
      <div class="flex flex-row items-center justify-start w-full space-x-4">
        <label for="description" class="w-24 text-center text-md font-medium text-gray-900 dark:text-white">{{translate("Description", "描述")}}</label>
        <textarea id="description" rows="4" type="text" class="px-2 grow border-[1px] border-black rounded-lg" v-model="video.description"/>
      </div>
      <div class="flex flex-row items-center justify-start w-full space-x-4">
        <label for="profile" class="w-24 min-w-fit text-center text-md font-medium text-gray-900 dark:text-white">{{translate("Poster", "海报")}}<span class="text-red-600">*</span></label>
        <input id="profile" type="text" class="px-2 grow border-[1px] border-black rounded-lg" v-model="video.poster_path"/>
      ...
      <div class="flex flex-row items-center justify-start w-full space-x-4">
        <label for="region" class="w-24 text-center text-md font-medium text-gray-900 dark:text-white">{{translate("Video Type", "视频类型")}}<span class="text-red-600">*</span></label>
      ...
      <div class="flex flex-row items-center justify-start w-full space-x-4">
        <label for="region" class="w-24 text-center text-md font-medium text-gray-900 dark:text-white">{{translate("Language", "语言")}}<span class="text-red-600">*</span></label>
      ...
      <div class="flex flex-row items-center justify-start w-full space-x-4">
        <label for="link_type" class="w-24 text-center text-md font-medium text-gray-900 dark:text-white">{{translate("Link Type", "链接类型")}}<span class="text-red-600">*</span></label>
        <select id="link_type" class="px-2 grow border-[1px] border-black rounded-lg" v-model="video.link_type">
          <option value="0" selected>{{translate("Unselected","未选择")}}</option>
      ...
      <div class="flex flex-row items-center justify-start w-full space-x-4" v-if="video.link_type==='1' || video.link_type==='2'">
        <label for="link" class="w-24 text-center text-md font-medium text-gray-900 dark:text-white">{{translate("Link", "链接")}}<span class="text-red-600">*</span></label>
      ...
      <div class="flex flex-row items-center justify-start w-full space-x-4" v-else-if="video.link_type==='3'">
        <label for="link_file" class="w-24 text-center text-md font-medium text-gray-900 dark:text-white">{{translate("File", "种子文件")}}<span class="text-red-600">*</span></label>
        <input id="link_file" type="file" class="grow"/>
      </div>
      <div class="flex flex-row items-center justify-start w-full space-x-4">
        <label for="profile" class="w-24 min-w-fit text-center text-md font-medium text-gray-900 dark:text-white">{{translate("Backdrop", "背景")}}<span class="text-red-600">*</span></label>
        <input id="profile" type="text" class="px-2 grow border-[1px] border-black rounded-lg" v-model="video.backdrop_path"/>
