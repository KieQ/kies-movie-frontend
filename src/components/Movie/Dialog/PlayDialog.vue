        ...
        <div class=" p-5 border-none w-11/12 md:w-96 shadow-lg rounded-md bg-gradient-to-br from-purple-200 to-indigo-400">
          <div class="mt-3 text-center">
            ...
            <div class="mt-2">
                <ul class="bg-yellow-100/40 max-h-36 overflow-y-auto rounded-lg">
                  <li class="m-1" v-for="(item, idx) of play_files" :key="idx">
                    <input type="radio" name="play_file" :id="`id_${item.display_path}`" :value="item.path" class="hidden peer play_file" required="">
            ...
            <div class="flex flex-row justify-center items-center px-4 py-3 space-x-4">
              <button class="px-4 py-2 text-white text-base font-medium rounded-md shadow-sm focus:outline-none focus:ring-2 grow bg-blue-500 hover:bg-blue-600 focus:ring-blue-300"
                      @click="click_play">
                {{translate("Play", "播放")}}
              </button>
